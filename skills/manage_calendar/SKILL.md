...
description: |
  管理 Google 行事曆（讀取摘要、新增行程、移除行程或更新行程），且具備「衝突預檢」功能。
  ...
  當工具回傳「偵測到行程衝突」及 `exit status 1` 錯誤時，AI **絕對不可以** 自動將 `force` 設為 `true` 再次執行。AI **必須** 向使用者回報衝突詳情（包含被佔用的行程名稱與時間），並**明確詢問**使用者是否確認要強制排入。只有當使用者回答「確認」或是「強制排入」等同意字眼後，AI 才能使用 `force=true` 再次呼叫工具。
...
* `force`: 若為 true，則強制新增行程，不進行衝突預檢。* **全天事件判定**：
...
#### 2.3 執行準則：衝突管理 (Conflict Management)
當你嘗試「新增」或「修改時間」時：
1. **預設執行**：先以 `force=false` 呼叫工具。
2. **處理衝突**：若工具因為衝突而回傳錯誤（通常包含「偵測到行程衝突」與 `exit status 1`）：
   - **立即停止操作**：**絕對不可以**立刻自行加上 `force=true` 重試。
   - **告知使用者**：明確列出是哪一個現有行程擋住了（包含名稱與時間）。
   - **提議方案**：詢問使用者是要「強制重疊（Force）」、「改約其他時間」還是「取消動作」。
   - **二次執行**：僅在接收到使用者**明確回覆**「沒關係」、「照樣排入」或「確認新增」時，才重新呼叫工具並帶入 `force=true`。

### 2.4 執行準則：衝突與空檔處理
1. **偵測衝突**：當你執行新增或更新，工具回傳「偵測到行程衝突」時，**暫停**後續動作。
2. **分析建議**：工具會同時輸出 `✅ 15:00 ~ 16:00` 類型的空檔建議。
3. **主動溝通**：
   - 先告知衝突的行程名稱。
   - **主動提議**：「原本的時段已有行程，我幫您查了今天還有三個空檔，分別是 15:00、16:30、19:00，您要改到哪一個？或是要強制排入？」
4. **執行改期**：根據使用者的選擇，再次呼叫 `mode=create` 或 `update` 並帶入新時間。

#### 步驟 B：時間自動補全
若使用者說「刪除三點的活動」，你應自動結合當前日期產出 `--from="2026-02-28T15:00:00"`。

### 2.5 更新行程執行準則 (Update Mode)
...
* **禁止放棄**：看到衝突時不要只說「不能排」，必須把工具給你的空檔建議告訴使用者。
* **精準時間**：空檔建議是基於活動原有的時長計算的，請直接引用。